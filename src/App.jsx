...
// ConfirmDialog — reemplaza window.confirm (Chrome lo suprime en tabs inactivos)
function ConfirmDialog({ msg, onOk, onCancel, okLabel="Confirmar", okClass="btn btnd btnfull" }) {
  return createPortal(
    <div className="overlay" onClick={onCancel}>
      <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:340}}>
        <div style={{fontSize:14,color:"#2C2C2C",lineHeight:1.6,marginBottom:20,marginTop:8}}>{msg}</div>
        <button className={okClass} onClick={onOk} style={{marginBottom:8}}>{okLabel}</button>
        <button className="btn btng btnfull" onClick={onCancel}>Cancelar</button>
      </div>
    </div>,
    document.body
  );
...
// ─── ACCESO POR PLAN ─────────────────────────────────────────────────────────
const isClinica = (user) => ["Clinica","Colegio"].includes(user?.plan) || user?.role === "admin" || user?.role === "admin_org";
  ...
  return (
    <div className="sidebar">
      ...
      {NAV.filter(n => {
        ...
        const isOrgAdmin = user?.role === "admin_org";
        // Admin global (Adriana): SOLO ve dashboard técnico, organizaciones, admin, perfil
        // NO ve datos clínicos (Ley 18.331 Uruguay)
        if (isAdmin && !["dashboard","organizaciones","admin","profile"].includes(n.id)) return false;
        // Solo superadmin ve adminOnly
        if (n.adminOnly && !isAdmin) return false;
        // admin_org: ve liquidación, organizaciones, perfil, y sus páginas clínicas propias
        if (isOrgAdmin && !["liquidacion","organizaciones","profile","patients","asistencias","dashboard","sessions","history","agenda"].includes(n.id)) return false;
...
function Patients({ patients, setPatients, setActive, setSelPatId, sessions, user={} }) {
  ...
  const [confirmElimPat, setConfirmElimPat] = useState(null);
      ...
      {sel && !editing && (
        <Modal title="" onClose={() => setSel(null)}>
          ...
          <div style={{ display:"flex", gap:7, flexWrap:"wrap", marginBottom:12 }}>
            ...
            <button className="btn btnd btnsm" onClick={() => setConfirmElimPat(sel)}>🗑️ Eliminar</button>
...
Hadrion — ${user?.email || ""}`;
      ...
      {confirmElimPat && (
        <ConfirmDialog
          msg={`¿Eliminar a ${confirmElimPat.name}?\n\nEsta acción no se puede deshacer.`}
          okLabel="Sí, eliminar"
          onOk={()=>{ setPatients(prev=>prev.filter(p=>p.id!==confirmElimPat.id)); setSel(null); setConfirmElimPat(null); }}
          onCancel={()=>setConfirmElimPat(null)}
        />
      )}
    </div>
  );
                ...
                <div key={p.id} style={{background:"white",borderRadius:14,padding:14,marginBottom:10,border:`1.5px solid ${C.terraL}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      ...
                      <button className="btn btnd btnsm" onClick={()=>{ if(window.prompt("Escribí ELIMINAR para confirmar") === "ELIMINAR") eliminarPersonalizada(p.id); }}>🗑️</button>
                    </div>
                  </div>
...
function Admin({ users, setUsers, registerRequests, setRegisterRequests, currentUser, precios=null, setPrecios=null }) {
  const isOrgAdmin = currentUser?.role === "admin_org";
  ...
  const del = async (id) => {
    ...
    // Eliminación suave: 30 días (sin window.confirm — Chrome lo suprime)
    const dataExpiresAt = new Date(Date.now() + 30*24*60*60*1000).toISOString().slice(0,10);
    try {
      await sbFetch(`hadrion_users?id=eq.${id}`, {
        method:"PATCH",
        body: JSON.stringify({ status:"cancelled", data_expires_at:dataExpiresAt, cancelled_at:new Date().toISOString() })
      });
    } catch(e) {
      alert("Error: " + e.message);
      return;
    }
    setUsers(prev => prev.map(u => u.id===id ? {...u, status:"cancelled", data_expires_at:dataExpiresAt} : u));
            ...
            <div key={u.id} className="sc" style={{ marginBottom:10 }}>
              <div className="sch">
                ...
                <div style={{ display:"flex", gap:5, alignItems:"center" }}>
                  <span className={`roletag ${u.role==="admin"?"radmin":"rpro"}`}>{u.role==="admin"?"👑 Admin":u.role==="admin_org"?"🏢 Admin Org":"Pro"}</span>
              ...
              <div className="scb">
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  ...
                  {u.id !== currentUser?.id && <button className="btn btnd btnsm" onClick={() => setConfirmDelUser(u)}>🗑️ Cancelar cuenta</button>}
                </div>
              </div>
            </div>
          ))}
...
Si les interesa, escríbanme 💜`}
      ...
      {confirmDelUser && (
        <ConfirmDialog
          msg={`¿Cancelar la cuenta de "${confirmDelUser.name}"?\n\nSus datos se conservarán 30 días. Después se eliminan permanentemente.`}
          okLabel="Sí, cancelar cuenta"
          onOk={()=>del(confirmDelUser.id)}
...
function Organizaciones({ users, setUsers, currentUser, precios={} }) {
  const PLANES_ORG = PLANES_ORG_BASE.map(p => ({ ...p, precio: precios[p.id] || 0 }));
  const isGlobalAdmin = currentUser?.role === "admin";
  const myOrgId       = currentUser?.org_id || null;

  const [orgs, setOrgs]     = useState([]);
  const [sbUsers, setSbUsers] = useState([]); // usuarios de Supabase (reales)
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // org a eliminar (30 días)
  const [errForm, setErrForm] = useState(""); // error visible en formulario
  const tiposOrg = ["Clinica","Escuela","Colegio","Centro terapeutico","Hospital","Otro"];

  // Cargar orgs y usuarios reales de Supabase al montar
  useEffect(() => {
    setSbLoading(true);
    Promise.all([
      sbFetch("hadrion_organizaciones?select=*&order=created_at.desc"),
      sbFetch("hadrion_users?select=*"),
    ]).then(([orgsData, usersData]) => {
      // admin_org: solo ve su propia org
      const filtradas = isGlobalAdmin
        ? (orgsData || [])
        : (orgsData || []).filter(o => o.id === myOrgId);
      setOrgs(filtradas);
      setSbUsers(usersData || []);
      setSbLoading(false);
  ...
  const addOrg = async () => {
    if (savingOrg) return;
    setErrForm("");
    // Validación visible en pantalla (no alert — Chrome lo suprime)
    if (!f.nombre.trim()) { setErrForm("Completá el nombre de la organización."); return; }
    if (!f.adminName.trim()) { setErrForm("Completá el nombre completo del administrador."); return; }
    if (!f.adminEmail.trim()) { setErrForm("Completá el email del administrador."); return; }
    if (!f.adminPassword.trim()) { setErrForm("Completá la contraseña del administrador."); return; }
    setSavingOrg(true);
    try {
      // Verificar que el email del admin no exista ya
      const existing = await sbFetch(`hadrion_users?email=eq.${encodeURIComponent(f.adminEmail.trim())}&select=id`);
      if (existing && existing.length > 0) {
        setErrForm("Ya existe un usuario con ese email. Usá otro email para el administrador.");
        return;
      }
      const orgId = crypto.randomUUID ? crypto.randomUUID() : makeId();
      const plan = PLANES_ORG.find(p => p.id === f.plan);
      const newOrg = {
        id: orgId, nombre: f.nombre.trim(), tipo: f.tipo, plan: f.plan,
        max_users: parseInt(f.maxUsers) || (plan?.maxUsers || 10),
        contacto: f.contacto.trim(), telefono: f.telefono.trim(),
        activa: true, created_at: new Date().toISOString(),
      };
      const adminName = f.adminName.trim();
      const init = adminName.split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,2) || "AD";
      const cols = [C.terra,C.sage,C.info,C.purple,C.gold];
      const adminUser = {
        name: adminName,                        // ← campo name limpio y no-null
        email: f.adminEmail.trim(),
        password: f.adminPassword.trim(),
        role: "admin_org",                      // ← rol correcto
        org_id: orgId, plan: f.plan, status: "active",
        specialty: "Administrador", phone: f.telefono.trim(),
      ...
      setErrForm("");
    } catch(e) { setErrForm("Error al guardar: " + e.message); }
  ...
  const deleteOrgSb = async (org) => {
    // Sin window.confirm — usar ConfirmDialog en render
    const dataExpiresAt = new Date(Date.now() + 30*24*60*60*1000).toISOString().slice(0,10);
    await sbFetch(`hadrion_organizaciones?id=eq.${org.id}`, {
      method:"PATCH", body: JSON.stringify({ activa:false, status:"cancelled", data_expires_at:dataExpiresAt, cancelled_at:new Date().toISOString() })
    }).catch(console.warn);
    await sbFetch(`hadrion_users?org_id=eq.${org.id}`, {
      method:"PATCH", body: JSON.stringify({ status:"cancelled", data_expires_at:dataExpiresAt })
    }).catch(console.warn);
    setOrgs(prev => prev.map(o => o.id === org.id ? {...o, activa:false, status:"cancelled", data_expires_at:dataExpiresAt} : o));
    setSelOrg(null);
    setConfirmDelete(null);
  };

  const orgUsers   = org => sbUsers.filter(u => u.org_id === org.id);
  const canAddUser = org => orgUsers(org).filter(u => u.status !== "cancelled").length < (org.max_users || 10);
    ...
    <div className="fu">
          ...
          <div key={org.id} style={{background:"white",borderRadius:18,overflow:"hidden",boxShadow:"0 2px 12px rgba(0,0,0,.07)",marginBottom:14}}>
            ...
            <div style={{padding:"14px 16px"}}>
              ...
              <div style={{marginTop:8}}>
                {miembros.map(u => (
                  <div key={u.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:"1px solid #F5F0FA",opacity:u.status==="cancelled"?.6:1}}>
                    <div style={{width:28,height:28,borderRadius:8,background:u.color||C.terra,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10,fontWeight:700,flexShrink:0}}>
                      {u.avatar||u.name?.slice(0,2).toUpperCase()}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:12,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{u.name}</div>
                      <div style={{fontSize:10,color:"#9B9590"}}>{u.email} · {u.specialty||"—"}</div>
                      {u.status === "cancelled" && (
                        <div style={{fontSize:10,color:"#C0392B",fontWeight:600}}>
                          Cancelado · datos hasta {u.data_expires_at ? new Date(u.data_expires_at).toLocaleDateString("es-UY") : "—"}
                        </div>
                      )}
                    </div>
                    <div style={{display:"flex",gap:4,flexShrink:0}}>
                      <span style={{fontSize:9,background:"#EDE0F5",color:"#9B7EBD",borderRadius:8,padding:"2px 6px",fontWeight:700}}>
                        {u.role==="admin_org"?"Admin":"Terapeuta"}
                      </span>
                      {/* Reenviar credenciales */}
                      <button onClick={()=>{
                        const phone = (u.phone||"").replace(/\D/g,"");
                        const msg = `Hola ${u.name?.split(" ")[0]} 👋\n\nTus datos de acceso a Hadrion:\n📧 Email: ${u.email}\n🔑 Contraseña: ${u.password||"(ver panel)"}\n\n👉 https://hadrion.pages.dev`;
                        window.open(waLink(phone,"598",msg),"_blank");
                      }} title="Reenviar credenciales por WhatsApp"
                        style={{background:"#E8F8EF",border:"none",borderRadius:7,padding:"3px 7px",cursor:"pointer",fontSize:12,lineHeight:1}}>
                        💬
                      </button>
                    </div>
                  </div>
                ...
                {miembros.length === 0 && (
                  <div style={{fontSize:12,color:"#9B9590",padding:"6px 0"}}>Sin usuarios aún.</div>
                )}
                {!canAddUser(org) && (
      ...
      {showNew && (
        <Modal title="Nueva Organización" onClose={()=>setShowNew(false)}>
          ...
          {errForm && <div className="alert alrtd" style={{marginBottom:12}}>⚠️ {errForm}</div>}
      ...
      {selOrg && (
        <Modal title={`⚙️ ${selOrg.nombre}`} onClose={()=>setSelOrg(null)}>
          ...
          {selOrg?.status !== "cancelled" ? (
            <button className="btn btnd btnfull" onClick={()=>{ setConfirmDelete(selOrg); setSelOrg(null); }}>
              🗑️ Cancelar organización
            </button>
          ) : (
            <div style={{fontSize:12,color:"#C0392B",background:"#FDECEA",borderRadius:10,padding:"8px 12px",marginBottom:8}}>
              ⚠️ Cancelada — datos se eliminan el {selOrg.data_expires_at ? new Date(selOrg.data_expires_at).toLocaleDateString("es-UY") : "—"}
            </div>
          )}
          <button className="btn btng btnfull" onClick={()=>setSelOrg(null)}>Cerrar</button>
        </Modal>
      )}

      {/* ConfirmDialog eliminación org — sin window.confirm */}
      {confirmDelete && (
        <ConfirmDialog
          msg={`¿Cancelar la organización "${confirmDelete.nombre}"?\n\nSus datos se conservarán 30 días. Después se eliminan permanentemente.`}
          okLabel="Sí, cancelar organización"
          onOk={()=>deleteOrgSb(confirmDelete)}
...
export default function HadrionApp() {
  ...
  // profesional → terapeuta: solo sus propios pacientes y asistencias.
  const isGlobalAdmin = user?.role === "admin";
  const isOrgAdmin    = user?.role === "admin_org";

  // Pantalla de acceso bloqueado — Ley 18.331 Uruguay + secreto profesional
  const PrivacyBlock = ({ motivo = "datos clínicos de pacientes" }) => (
    <div className="fu">
      <div style={{background:"linear-gradient(135deg,#FDF0E8,#FAF8F5)",borderRadius:18,padding:32,textAlign:"center",marginTop:20,border:"2px solid #EDE0F5"}}>
        <div style={{fontSize:48,marginBottom:12}}>🔒</div>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:"#2C2C2C",marginBottom:8}}>Acceso restringido</div>
        <div style={{fontSize:13,color:"#6B6560",lineHeight:1.8,maxWidth:320,margin:"0 auto"}}>
          Los <strong>{motivo}</strong> están protegidos por el <strong>secreto profesional</strong> y
          la <strong>Ley 18.331</strong> de Protección de Datos Personales (Uruguay).<br/><br/>
          Solo el profesional a cargo y el equipo clínico autorizado pueden acceder a esta información.
        </div>
      </div>
    </div>
  );
  ...
  const pages = {
    // Admin global → dashboard técnico (sin datos de pacientes)
    dashboard: isGlobalAdmin
      ? <div className="fu">
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,fontWeight:700,color:C.terra,marginBottom:4}}>Panel de administración</div>
          <div style={{fontSize:13,color:C.grayL,marginBottom:20}}>Gestión de organizaciones y usuarios · Hadrion</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:16}}>
            {[["🏫","Organizaciones",users.filter(u=>u.org_id).length+" en orgs"],["👤","Usuarios",users.length+" registrados"],["✅","Activos",users.filter(u=>u.status==="active").length+""],["⚠️","Cancelados",users.filter(u=>u.status==="cancelled").length+""]].map(([i,t,v])=>(
              <div key={t} style={{background:"white",borderRadius:14,padding:"14px 16px",boxShadow:"0 1px 6px rgba(0,0,0,.06)"}}>
                <div style={{fontSize:24,marginBottom:4}}>{i}</div>
                <div style={{fontWeight:700,fontSize:13}}>{t}</div>
                <div style={{fontSize:12,color:C.grayL,marginTop:2}}>{v}</div>
              </div>
            ))}
          </div>
          <button className="btn btnp btnfull" onClick={()=>setActive("organizaciones")}>🏫 Organizaciones</button>
          <button className="btn btng btnfull" style={{marginTop:8}} onClick={()=>setActive("admin")}>🔐 Panel de admin</button>
        </div>
      : <Dashboard user={user} patients={patients} sessions={sessions} payments={payments} setActive={setActive} setShowQS={setShowQS} agendaItems={agendaItems} />,

    // Datos clínicos — BLOQUEADOS para admin global (Ley 18.331 + secreto profesional Uruguay)
    agenda:     isGlobalAdmin ? <PrivacyBlock motivo="datos de agenda y pacientes" />
                : <Agenda patients={patients} items={agendaItems} setItems={setAgenda} />,
    patients:   isGlobalAdmin ? <PrivacyBlock />
                : <Patients patients={patients} setPatients={setPatients} setActive={setActive} setSelPatId={setSelPatId} sessions={sessions} />,
    payments:   isGlobalAdmin ? <PrivacyBlock motivo="datos de pagos de pacientes" />
                : <Payments patients={patients} payments={payments} setPayments={setPayments} />,
    sessions:   isGlobalAdmin ? <PrivacyBlock motivo="registros de sesiones clínicas" />
                : <Sessions patients={patients} sessions={sessions} setSessions={setSessions} setPatients={setPatients} user={user} />,
    history:    isGlobalAdmin ? <PrivacyBlock motivo="historial clínico de pacientes" />
                : <History patients={patients} sessions={sessions} selectedPatientId={selPatId} setPatients={setPatients} user={user} />,
    objectives: isGlobalAdmin ? <PrivacyBlock motivo="objetivos terapéuticos" />
                : <GoalBank user={user}/>,
    activities: isGlobalAdmin ? <PrivacyBlock motivo="actividades clínicas" />
                : <Activities user={user}/>,
    phonology:  isGlobalAdmin ? <PrivacyBlock motivo="datos de fonología" />
                : <Phonology />,
    reports:    isGlobalAdmin ? <PrivacyBlock motivo="informes clínicos" />
                : <Reports patients={patients} sessions={sessions} payments={payments} user={user} />,
    plan:       isGlobalAdmin ? <PrivacyBlock motivo="plan colaborativo clínico" />
                : <PlanColaborativo patients={patients} users={users} plan={plan} setPlan={setPlan} />,
    resources:  isGlobalAdmin ? <PrivacyBlock motivo="recursos y documentos clínicos" />
                : <Resources plantillas={plantillas} setPlantillas={setPlantillas} documentos={documentos} setDocumentos={setDocumentos} user={user}/>,
    tea:        isGlobalAdmin ? <PrivacyBlock motivo="recursos clínicos TEA" />
                : <TEAAutismo />,
    ia:         isGlobalAdmin ? <PrivacyBlock motivo="asistente clínico con datos de pacientes" />
                : <IAAsistente patients={patients} C={C} documentos={documentos} setDocumentos={setDocumentos} chatHistory={chatHistory} setChatHistory={setChatHistory}/>,
    psicologia: isGlobalAdmin ? <PrivacyBlock motivo="registros de psicología" />
                : <Psicologia patients={patients} sessions={sessions} documentos={documentos} setDocumentos={setDocumentos} datos={psicoDatos} setDatos={setPsicoDatos}/>,
    tcc:        isGlobalAdmin ? <PrivacyBlock motivo="registros de TCC" />
                : <TCC patients={patients} documentos={documentos} setDocumentos={setDocumentos} datos={tccDatos} setDatos={setTccDatos}/>,
    asistencias: isGlobalAdmin ? <PrivacyBlock motivo="asistencias de pacientes" />
                : <Asistencias patients={patients} setPatients={setPatients} />,

    // Organizaciones — admin global gestiona todas; admin_org ve solo la suya
    organizaciones: (isGlobalAdmin || isOrgAdmin || isClinica(user))
      ? <Organizaciones users={users} setUsers={setUsers} currentUser={user} precios={precios} />
      : <div className="fu">
          <div style={{background:"linear-gradient(135deg,#F5F0FA,#EDE0F5)",borderRadius:18,padding:28,textAlign:"center",marginTop:20}}>
            ...
            <div style={{fontSize:13,color:"#6B6560",lineHeight:1.7,marginBottom:16}}>Disponible en Plan Clínica o Colegio.</div>
    ...
    // Panel admin técnico — SOLO Adriana
    admin: isGlobalAdmin
      ...
      : <div className="fu"><div className="alert alrtd">🔐 Solo administradores técnicos.</div></div>,

    // Liquidación:
    // admin global → BLOQUEADO (contiene datos de pacientes)
    // admin_org   → habilitado: ve pacientes + asistencias + tarifa de sus terapeutas (sin diagnóstico ni notas)
    // profesional Plan Clínica → habilitado
    liquidacion: isGlobalAdmin
      ? <PrivacyBlock motivo="liquidaciones (contienen datos de pacientes)" />
      : (isOrgAdmin || isClinica(user))
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <div className="mwrap">
          {showCancelBanner && (
            <div style={{background:"#FDECEA",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,borderBottom:"1px solid #F5BCBC"}}>
              <div style={{fontSize:12,color:"#C0392B",fontWeight:600}}>
                ⚠️ Cuenta cancelada — datos se eliminan en <strong>{dataExpDays} días</strong> ({new Date(user.dataExpiresAt).toLocaleDateString("es-UY")}).
              </div>
              <a href="https://wa.me/59899926775?text=Hola%20Adriana%2C%20quiero%20reactivar%20mi%20cuenta%20de%20Hadrion"
                target="_blank" rel="noopener noreferrer"
                style={{background:"#25D366",color:"white",borderRadius:8,padding:"5px 12px",fontWeight:700,fontSize:11,textDecoration:"none"}}>
                💬 Reactivar
              </a>
            </div>
          )}
