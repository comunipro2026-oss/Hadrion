...
.phbtn{aspect-ratio:1;border-radius:16px;border:2px solid #EDE0F5;background:white;font-family:'Cormorant Garamond',serif;font-size:36px;font-weight:700;cursor:pointer;transition:all .17s;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#2C2C2C;gap:6px;min-height:90px;}
...
const NAV = [
  ...
  { id:"phonology",  l:"Conciencia Fonologica",   i:"🔤" },
        ...
        <div style={{ background:C.terraF, borderRadius:16, padding:16, marginBottom:12, border:`2px solid ${C.terraL}` }}>
            ...
            <div style={{ background:"white", borderRadius:12, padding:14, marginBottom:10 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
                {emojis.slice(0,6).map((emoji, i) => (
                  <div key={i} style={{ textAlign:"center", cursor:"pointer", background:C.terraF, borderRadius:12, border:`2px solid ${C.terraL}`, width:80, padding:"8px 4px" }}
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
                {emojis.slice(0,4).map((emoji, i) => (
                  <div key={i} style={{ textAlign:"center", cursor:"pointer", background:"white", borderRadius:12, border:`2px solid ${C.sand}`, width:80, padding:"8px 4px" }}
                    ...
                    <div style={{ fontSize:36 }}>{emoji}</div>
                    <div style={{ fontSize:10, fontWeight:700, color:C.gray, marginTop:4 }}>{words[i]||""}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {stage === "Letra" && (
            <div style={{ background:"white", borderRadius:12, padding:14, marginBottom:10 }}>
              ...
              <div style={{ textAlign:"center", marginBottom:12 }}>
                <button style={{ background:C.terra, color:"white", border:"none", borderRadius:12, padding:"12px 24px", fontSize:14, fontWeight:700, cursor:"pointer" }} onClick={() => speak(sel)}>🔊 Escuchar sonido</button>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
                {[sel, ...PHONEMES.filter(p=>p!==sel).sort(()=>Math.random()-0.5).slice(0,2)].sort(()=>Math.random()-0.5).map((ph,i) => (
                  <button key={i} onClick={(e) => { e.currentTarget.style.background=ph===sel?"#E8F8EF":"#FDECEA"; e.currentTarget.style.border=ph===sel?"2px solid #27AE60":"2px solid #C0392B"; if(ph===sel)setScore(s=>s+1); }}
                    style={{ padding:"16px", borderRadius:14, border:`2px solid ${C.sand}`, background:"white", fontFamily:"Georgia,serif", fontSize:28, fontWeight:700, cursor:"pointer", color:C.charcoal }}>
                    {ph}
                  </button>
                ))}
              </div>
          ...
          {stage === "Silaba" && (
            <div style={{ background:"white", borderRadius:12, padding:14, marginBottom:10 }}>
              ...
              {words.slice(0,4).map((w, i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px", background:C.terraF, borderRadius:12, marginBottom:8, cursor:"pointer" }} onClick={() => speakWord(w)}>
                  <div style={{ fontSize:28 }}>{emojis[i]||"📝"}</div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700, fontSize:18, color:C.charcoal, letterSpacing:4 }}>{w}</div>
                    <div style={{ fontSize:11, color:C.terra, marginTop:2 }}>👏 toca para escuchar</div>
                  </div>
                </div>
              ))}
            </div>
              ...
              {words.slice(0,3).map((w, i) => (
                <div key={i} style={{ marginBottom:10, background:C.terraF, borderRadius:12, padding:12 }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {w.split("").map((letra, j) => (
                      <div key={j} style={{ width:36, height:36, borderRadius:8, border:`2px solid ${C.terraL}`, background:"white", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Georgia,serif", fontSize:18, fontWeight:700, color:C.terra }}>
                        {letra.toUpperCase()}
                      </div>
                    ))}
                  </div>
              ...
              {words.slice(0,3).map((w, i) => (
                <div key={i} style={{ marginBottom:10, background:C.terraF, borderRadius:12, padding:12 }}>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:6 }}>
                    {w.split("").map((letra, j) => (
                      <button key={j} style={{ minWidth:36, height:36, borderRadius:8, border:`2px solid ${C.terraL}`, background:"white", fontFamily:"Georgia,serif", fontSize:16, fontWeight:700, color:C.terra, cursor:"pointer" }} onClick={() => speak(letra)}>
                        {letra.toUpperCase()}
                      </button>
                    ))}
                    <button style={{ height:36, borderRadius:8, border:"none", background:C.terra, color:"white", padding:"0 12px", fontSize:12, fontWeight:700, cursor:"pointer" }} onClick={() => speakWord(w)}>
                      🔊 Completa
                    </button>
                  </div>
                </div>
      ...
      <div className="phgrid">
        {(cats[fil] || PHONEMES).map(ph => (
          <button key={ph} className={`phbtn${sel===ph?" sel":""}`} onClick={() => handle(ph)}>
            ...
            {PHONEME_EMOJI[ph] && <div style={{ fontSize:30, lineHeight:1 }}>{PHONEME_EMOJI[ph][0]}</div>}
          </button>
        ))}
      </div>
...
export default function HadrionApp() {
  ...
  const iniciarAtrapa=()=>{
    const ph=shuffle(phonemes).slice(0,8);
    setRonda(ph);setIdx(0);setScore(0);setFeedback(null);setJuegoActivo("atrapa");
    setTimeout(()=>speak(ph[0]),500);
  };

  const iniciarDonde=()=>{
    const ph=shuffle(phonemes.filter(p=>phonemeEmoji[p]&&phonemeEmoji[p].length>0)).slice(0,6);
    setRonda(ph);setIdx(0);setScore(0);setFeedback(null);setJuegoActivo("donde");
  };

  const responderAtrapa=(ph)=>{
    if(feedback)return;
    const ok=ph===ronda[idx];
    setFeedback(ok?"ok":"mal");
    if(ok)setScore(s=>s+1);
    setTimeout(()=>{
      setFeedback(null);
      if(idx+1<ronda.length){setIdx(i=>i+1);setTimeout(()=>speak(ronda[idx+1]),300);}
      else setJuegoActivo("fin");
    },900);
  };
  ...
  if(juegoActivo==="atrapa"&&idx<ronda.length){
    ...
    return(
      <div style={{background:"white",borderRadius:16,padding:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
          {opciones.map((ph,i)=>(
            <button key={i} onClick={()=>responderAtrapa(ph)} style={{padding:"18px 8px",borderRadius:14,border:"2px solid #EDE0F5",background:"white",fontFamily:"Georgia,serif",fontSize:28,fontWeight:700,cursor:"pointer",color:"#2C2C2C"}}>{ph}</button>
          ))}
        </div>
  ...
  if(juegoActivo==="donde"&&idx<ronda.length){
    ...
    const opts=shuffle([
      {ph:correctPh,emoji:(phonemeEmoji[correctPh]||[])[0]||"📝",word:(phonemeWords[correctPh]||[])[0]||correctPh},
      ...wrongPhs.map(p=>({ph:p,emoji:(phonemeEmoji[p]||[])[0]||"📝",word:(phonemeWords[p]||[])[0]||p}))
    ]);
    return(
      <div style={{background:"white",borderRadius:16,padding:16}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {opts.map((opt,i)=>(
            <button key={i} onClick={()=>{
              if(feedback)return;
              const ok=opt.ph===correctPh;
              setFeedback(ok?"ok":"mal");
              if(ok)setScore(s=>s+1);
              setTimeout(()=>{
                setFeedback(null);
                if(idx+1<ronda.length)setIdx(i=>i+1);
                else setJuegoActivo("fin");
              },900);
            }} style={{padding:"14px 8px",borderRadius:12,border:"2px solid #F9C8DC",background:"white",cursor:"pointer",textAlign:"center"}}>
  ...
  if(juegoActivo==="fin"){
    return(
      <div style={{background:"white",borderRadius:16,padding:24,textAlign:"center"}}>
        ...
        <div style={{background:"#F5F0FA",borderRadius:12,padding:12,marginBottom:16,fontSize:13}}>
          {score===ronda.length?"🌟 Perfecto! Excelente trabajo!":score>=ronda.length/2?"👏 Muy bien! Segui practicando!":"💪 Buen intento! Seguimos!"}
        </div>
  ...
  return(
    <div>
      {[
        {id:"atrapa",icon:"🎯",name:"Atrapa el sonido",desc:"Escucha el fonema y toca la letra correcta",color:"#9B7EBD",action:iniciarAtrapa},
        {id:"donde",icon:"❓",name:"Donde esta?",desc:"Ve el fonema y toca la imagen que empieza con ese sonido",color:"#E8719C",action:iniciarDonde},
      ].map(j=>(
        <div key={j.id} style={{display:"flex",alignItems:"center",gap:12,padding:12,background:"white",borderRadius:14,marginBottom:10,cursor:"pointer",border:`2px solid ${j.color}22`,boxShadow:"0 1px 6px rgba(0,0,0,.06)"}} onClick={j.action}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:"#9B9590",marginTop:2}}>{j.desc}</div>
          </div>
