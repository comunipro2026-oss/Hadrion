...
@media(min-width:500px){.phgrid{grid-template-columns:repeat(5,1fr);}}
.phbtn{aspect-ratio:1;border-radius:14px;border:2px solid #EDE0F5;background:white;font-family:'Cormorant Garamond',serif;font-size:26px;font-weight:700;cursor:pointer;transition:all .17s;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#2C2C2C;gap:4px;min-height:72px;}
      ...
      <div className="phgrid">
        {(cats[fil] || PHONEMES).map(ph => (
          <button key={ph} className={`phbtn${sel===ph?" sel":""}`} onClick={() => handle(ph)}>
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:18, fontWeight:700 }}>{ph}</div>
            {PHONEME_EMOJI[ph] && <div style={{ fontSize:22, lineHeight:1 }}>{PHONEME_EMOJI[ph][0]}</div>}
          </button>
        ))}
      </div>
