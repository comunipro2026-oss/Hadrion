...
function Agenda({ patients, items, setItems }) {
  ...
  const save = () => {
    ...
    setItems(prev => [...prev, { id:Date.now(), patient:f.patient, time:f.time, end, type:f.type, color:p?.color || C.terra, date:f.date }]);
...
function Payments({ patients, payments, setPayments }) {
  ...
  const save = () => {
    ...
    setPayments(prev => [...prev, { id:Date.now(), patientId:parseInt(f.patientId), patient:p?.name||"", amount:parseInt(f.amount), type:f.type, date:f.date, method:f.method, status:"pagado" }]);
...
function Sessions({ patients, sessions, setSessions, setPatients }) {
  ...
  const save = () => {
    ...
    const p = patients.find(x => x.id === parseInt(f.patientId));
    const newSession = { id:Date.now(), patientId:parseInt(f.patientId), patient:p?.name||"", date:new Date().toLocaleDateString("es-UY"), ...f, activities:f.activities ? f.activities.split(",").map(s => s.trim()) : [] };
      ...
      {sel && (
        <div style={{background:C.terraF,borderRadius:16,padding:16,marginBottom:12,border:`2px solid ${C.terraL}`}}>
          <div style={{textAlign:"center",marginBottom:10}}>
            ...
            <button className="btn btnp btnsm" style={{marginTop:6}} onClick={()=>speak(sel)}>🔊 Escuchar</button>
          </div>

          {stage==="Escucha"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:13,color:C.terraD,marginBottom:8}}>🎧 Escucha y repeti en voz alta</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center"}}>
              {emojis.slice(0,6).map((emoji,i)=>(
                <div key={i} style={{textAlign:"center",cursor:"pointer",background:C.terraF,borderRadius:12,border:`2px solid ${C.terraL}`,width:80,padding:"8px 4px"}} onClick={()=>speakWord(words[i]||sel)}>
                  <div style={{fontSize:36}}>{emoji}</div>
                  <div style={{fontSize:11,fontWeight:800,color:C.terra,marginTop:4}}>{words[i]||""}</div>
                </div>
              ))}
            </div>
          </div>}

          {stage==="Imagen"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:13,color:C.terraD,marginBottom:6}}>🖼️ Toca las imagenes que empiezan con {sel}</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center"}}>
              {emojis.slice(0,6).map((emoji,i)=>(
                <div key={i} style={{textAlign:"center",cursor:"pointer",background:"white",borderRadius:12,border:`2px solid ${C.sand}`,width:80,padding:"8px 4px"}}
                  onClick={(e)=>{e.currentTarget.style.border="2px solid #27AE60";e.currentTarget.style.background="#E8F8EF";setScore(s=>s+1);speakWord(words[i]||sel);}}>
                  <div style={{fontSize:36}}>{emoji}</div>
                  <div style={{fontSize:11,fontWeight:700,color:C.gray,marginTop:4}}>{words[i]||""}</div>
                </div>
              ))}
            </div>
          </div>}

          {stage==="Letra"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
          ...
          {stage==="Silaba"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
          ...
          {stage==="Segmentacion"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
            ...
            {words.slice(0,3).map((w,i)=>(
              <div key={i} style={{marginBottom:10,background:C.terraF,borderRadius:12,padding:12}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                  <div style={{fontSize:28}}>{emojis[i]||"📝"}</div>
                  <div style={{fontFamily:"Georgia,serif",fontSize:20,fontWeight:700,color:C.charcoal}}>{w}</div>
                  <button style={{marginLeft:"auto",background:"#4285F4",color:"white",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,cursor:"pointer"}} onClick={()=>speakWord(w)}>🔊</button>
                </div>
          ...
          {stage==="Fusion"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
            ...
            {words.slice(0,3).map((w,i)=>(
              <div key={i} style={{marginBottom:10,background:C.terraF,borderRadius:12,padding:12}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  ...
                  <button style={{height:36,borderRadius:8,border:"none",background:C.terra,color:"white",padding:"0 12px",fontSize:12,fontWeight:700,cursor:"pointer"}} onClick={()=>speakWord(w)}>🔊 Completa</button>
                </div>
              </div>
            ))}
          </div>}

          {stage==="Manipulacion"&&<div style={{background:"white",borderRadius:12,padding:12,marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:13,color:C.terraD,marginBottom:8}}>🔀 Cambia el fonema inicial</div>
            {[["PATO","GATO"],["PISO","BISO"],["CAMA","TAMA"],["LUNA","TUNA"]].map(([w1,w2],i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,background:C.terraF,borderRadius:12,padding:"10px 14px"}}>
                ...
                <div style={{color:C.grayL,fontSize:18}}>→</div>
                ...
                <div style={{fontSize:11,color:C.grayL}}>Cambia {w1[0]} por {w2[0]}</div>
              </div>
            ))}
          </div>}
          ...
          <div style={{marginTop:8}}>
            ...
            {words.length>0&&(
              <div style={{display:"flex",alignItems:"center",gap:12,background:"white",borderRadius:12,padding:"10px 14px",marginBottom:6}}>
                <div style={{fontSize:36}}>{emojis[wordIdx%emojis.length]||"📝"}</div>
                <div style={{flex:1}}><div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:700,color:C.charcoal}}>{words[wordIdx%words.length]}</div></div>
      ...
      <div className="phgrid">
        {(cats[fil] || PHONEMES).map(ph => (
          <button key={ph} className={`phbtn${sel===ph?" sel":""}`} onClick={() => handle(ph)}>
            ...
            {PHONEME_EMOJI[ph] && <div style={{ fontSize:28, lineHeight:1, marginTop:2 }}>{PHONEME_EMOJI[ph][0]}</div>}
          </button>
        ))}
      </div>
...
function Resources(){
  ...
  const recursos=[
    {i:"📋",t:"Plantilla de anamnesis",d:"Formulario completo inicial",c:"Evaluacion",
     contenido:"PLANTILLA DE ANAMNESIS\n\nPaciente: _______________\nFecha nacimiento: _______________\nDiagnostico: _______________\nTutor: _______________\nTelefono: _______________\n\nMOTIVO DE CONSULTA\n________________________________\n\nANTECEDENTES\n- Embarazo/parto: _______________\n- Hitos desarrollo: _______________\n- Antecedentes medicos: _______________\n\nOBSERVACIONES\n________________________________\n\nHadrion - hadrion.netlify.app"},
    {i:"🎯",t:"Guia de objetivos TEL",d:"Objetivos por nivel TEL",c:"Lenguaje",
     contenido:"OBJETIVOS TERAPEUTICOS - TEL\n\nNIVEL INICIAL\n- Comprension de consignas simples\n- Vocabulario basico 50-100 palabras\n- Emision de palabras aisladas\n\nNIVEL MEDIO\n- Frases de 2-3 palabras\n- Vocabulario 200+ palabras\n- Discriminacion auditiva\n\nNIVEL AVANZADO\n- Oraciones completas SVO\n- Narrativa secuenciada\n- Conciencia fonologica\n\nHadrion - hadrion.netlify.app"},
    {i:"📊",t:"Escala de progreso",d:"Seguimiento visual del avance",c:"Seguimiento",
     contenido:"ESCALA DE PROGRESO\n\nPaciente: _______________\nPeriodo: _______________\n\nOBJETIVO 1: _______________\nProgreso: 10% 25% 50% 75% 100%\n\nOBJETIVO 2: _______________\nProgreso: 10% 25% 50% 75% 100%\n\nOBJETIVO 3: _______________\nProgreso: 10% 25% 50% 75% 100%\n\nFirma: _______________\nHadrion - hadrion.netlify.app"},
    {i:"🏠",t:"Guia actividades en casa",d:"Manual para familias",c:"Familia",
     contenido:"GUIA PARA FAMILIAS\n\nDURANTE EL BANO\n- Nombrar partes del cuerpo\n- Cantar canciones con gestos\n\nEN LA COCINA\n- Nombrar ingredientes\n- Describir lo que hacemos\n\nEN EL AUTO\n- Buscar letras en carteles\n- Contar objetos de un color\n\nANTES DE DORMIR\n- Leer cuento 5 minutos\n- Preguntar que paso\n\nContacto: comunipro12@gmail.com"},
    {i:"🔤",t:"Laminas fonologicas",d:"Imagenes por fonema",c:"Fonologia",
     contenido:"LAMINAS FONOLOGICAS\n\nVer la seccion Conciencia Fonologica en la app para ejercicios interactivos con audio.\n\nLaminas imprimibles disponibles en:\nhadrion.netlify.app/laminas.html\n\nContenido incluido:\n- Fonema A: Avion, Arbol, Araña\n- Fonema S: Sol, Silla, Sapo\n- Rimas, Silabas, Semaforo\n- Praxias, Vocabulario\n- Guia para familia"},
    {i:"📝",t:"Registro de sesion",d:"Plantilla editable",c:"Administracion",
     contenido:"REGISTRO DE SESION\n\nPaciente: _______________\nFecha: _______________\nSesion N: _____\n\nOBJETIVO TRABAJADO\n________________________________\n\nACTIVIDADES\n1. _______________\n2. _______________\n\nOBSERVACIONES\n________________________________\n\nPROGRESO (0-100%): _____\n\nTAREA PARA CASA\n________________________________\n\nFirma: _______________\nHadrion - hadrion.netlify.app"},
    {i:"📬",t:"Carta para escuela",d:"Modelo de comunicacion",c:"Derivacion",
     contenido:"INFORME PARA ESTABLECIMIENTO EDUCATIVO\n\nFecha: _______________\n\nSr/Sra Director/a:\n\nMe dirijo a usted para informar que el/la alumno/a _______________, concurre actualmente a tratamiento de _______________ desde _______________.\n\nSe sugiere:\n- _______________\n- _______________\n\nQuedo a su disposicion.\n\nAtentamente,\n_______________\nEmail: comunipro12@gmail.com\nHadrion - hadrion.netlify.app"},
    {i:"💡",t:"Estrategias TDAH",d:"Recomendaciones docentes",c:"TDAH",
     contenido:"ESTRATEGIAS PARA TDAH\n\nPARA EL AULA\n- Ubicar cerca del docente\n- Instrucciones cortas y claras\n- Dividir tareas en pasos\n- Tiempo extra en evaluaciones\n\nPARA LA FAMILIA\n- Rutinas fijas\n- Espacio sin distractores\n- Descansos cada 20 minutos\n- Reforzar logros positivamente\n\nAUTORREGULACION\n- Semaforo de emociones\n- Respiracion consciente\n\nHadrion - hadrion.netlify.app"},
  ];
  return(
    <div className="fu">
      <div style={{marginBottom:14}}><div className="pt">Recursos</div><div className="ps">Toca cada recurso para ver el contenido</div></div>
      <div className="alert alrti" style={{marginBottom:14}}>Toca cualquier recurso para ver e imprimir el contenido completo.</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {recursos.map((r,i)=>(
          <div key={i} className="card" style={{cursor:"pointer",padding:12}} onClick={()=>setSel(r)}>
            ...
            <div style={{fontWeight:700,fontSize:12,color:"#2C2C2C",marginBottom:3}}>{r.t}</div>
            <div style={{fontSize:11,color:"#9B9590",marginBottom:7}}>{r.d}</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span className="badge" style={{background:"#F5F0FA",color:"#9B7EBD",fontSize:10}}>{r.c}</span>
              <span style={{fontSize:10,color:"#5B8DB8",fontWeight:600}}>ver →</span>
            </div>
          </div>
        ))}
      </div>
      {sel&&(
        <div className="overlay" onClick={()=>setSel(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{background:"#FAF8F5",borderRadius:12,padding:14,marginBottom:12,whiteSpace:"pre-wrap",fontSize:12,color:"#2C2C2C",lineHeight:1.8,maxHeight:"50vh",overflowY:"auto"}}>{sel.contenido}</div>
            <button className="btn btnp btnfull noprint" onClick={()=>window.print()}>🖨️ Imprimir / PDF</button>
          </div>
        </div>
      )}
    </div>
...
function PlanColaborativo({ patients, users, plan, setPlan }) {
  ...
  const save = () => {
    ...
    setPlan(prev => [...prev, { id:Date.now(), patientId:parseInt(pid), ...f, objectives:f.objectives.split(",").map(o=>o.trim()).filter(Boolean), lastUpdate:new Date().toLocaleDateString("es-UY") }]);
...
function Admin({ users, setUsers, registerRequests, setRegisterRequests, currentUser }) {
  ...
  const add = () => {
    ...
    const init = f.name.split(" ").map(w => w[0]).join("").slice(0,2).toUpperCase();
    setUsers(prev => [...prev, { id:Date.now(), name:f.name, email:f.email, password:f.password, role:f.role, specialty:f.specialty, plan:f.plan, status:"active", createdAt:new Date().toLocaleDateString("es-UY"), avatar:init, color:cols[prev.length % cols.length], lastLogin:"—" }]);
                ...
                {r.status === "pendiente" && (
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    <button className="btn btnp btnsm" onClick={() => {
                      ...
                      setUsers(prev => [...prev, { id:Date.now(), name:r.name, email:r.email, password:"Hadrion" + Math.floor(1000+Math.random()*9000), role:"profesional", specialty:r.specialty, plan:"Basico", status:"active", createdAt:new Date().toLocaleDateString("es-UY"), avatar:init, color:cols[users.length%cols.length], lastLogin:"—" }]);
...
export default function HadrionApp() {
  ...
  const handleRegisterRequest = (form) => {
    setReg(prev => [...prev, { id:Date.now(), date:new Date().toLocaleDateString("es-UY"), ...form, status:"pendiente" }]);
  };
  ...
  const saveQS = () => {
    ...
    const newSess = { id:Date.now(), patientId:parseInt(qsF.patientId), patient:p?.name||"", date:new Date().toLocaleDateString("es-UY"), objective:"Sesion rapida", note:qsF.note, progress:50, activities:[], homework:"", estado:"", atencion:"", participacion:"", sensorial:[] };
...
function JuegosInteractivos({phonemes,phonemeEmoji,phonemeWords,C}){
  const [juegoActivo,setJuegoActivo]=React.useState(null);
  ...
  const iniciarAtrapa=()=>{const ph=shuffle(phonemes).slice(0,8);setRonda(ph);setIdx(0);setScore(0);setFeedback(null);setJuegoActivo("atrapa");setTimeout(()=>speak(ph[0]),500);};
  const iniciarDonde=()=>{const ph=shuffle(phonemes.filter(p=>phonemeEmoji[p]&&phonemeEmoji[p].length>0)).slice(0,6);setRonda(ph);setIdx(0);setScore(0);setFeedback(null);setJuegoActivo("donde");};
  const responderAtrapa=(ph)=>{if(feedback)return;const ok=ph===ronda[idx];setFeedback(ok?"ok":"mal");if(ok)setScore(s=>s+1);setTimeout(()=>{setFeedback(null);if(idx+1<ronda.length){setIdx(i=>i+1);setTimeout(()=>speak(ronda[idx+1]),300);}else setJuegoActivo("fin");},900);};
  const cerrar=()=>{setJuegoActivo(null);setFeedback(null);};
  if(juegoActivo==="atrapa"&&idx<ronda.length){
    const opciones=shuffle([ronda[idx],...shuffle(phonemes.filter(p=>p!==ronda[idx])).slice(0,2)]);
    return(<div style={{background:"white",borderRadius:16,padding:16}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
        <div style={{fontWeight:700,fontSize:14}}>🎯 Atrapa el sonido</div>
      ...
      <div style={{background:"#F5F0FA",borderRadius:14,padding:16,textAlign:"center",marginBottom:12}}>
        ...
        <button style={{background:"#9B7EBD",color:"white",border:"none",borderRadius:12,padding:"12px 28px",fontSize:15,fontWeight:700,cursor:"pointer"}} onClick={()=>speak(ronda[idx])}>🔊 {ronda[idx]}</button>
      </div>
      {feedback&&<div style={{textAlign:"center",padding:10,borderRadius:12,marginBottom:10,background:feedback==="ok"?"#E8F8EF":"#FDECEA",color:feedback==="ok"?"#27AE60":"#C0392B",fontWeight:700,fontSize:16}}>{feedback==="ok"?"✅ Correcto!":"❌ Incorrecto!"}</div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
        {opciones.map((ph,i)=>(<button key={i} onClick={()=>responderAtrapa(ph)} style={{padding:"18px 8px",borderRadius:14,border:"2px solid #EDE0F5",background:"white",fontFamily:"Georgia,serif",fontSize:28,fontWeight:700,cursor:"pointer",color:"#2C2C2C"}}>{ph}</button>))}
      </div>
      <button style={{marginTop:12,width:"100%",padding:10,borderRadius:10,border:"none",background:"#EDE0F5",color:"#6B6560",cursor:"pointer",fontWeight:600}} onClick={cerrar}>Salir</button>
    </div>);
  }
  if(juegoActivo==="donde"&&idx<ronda.length){
    const correctPh=ronda[idx];
    const wrongPhs=shuffle(phonemes.filter(p=>p!==correctPh&&phonemeEmoji[p]&&phonemeWords[p])).slice(0,3);
    const opts=shuffle([{ph:correctPh,emoji:(phonemeEmoji[correctPh]||[])[0]||"📝",word:(phonemeWords[correctPh]||[])[0]||correctPh},...wrongPhs.map(p=>({ph:p,emoji:(phonemeEmoji[p]||[])[0]||"📝",word:(phonemeWords[p]||[])[0]||p}))]);
    return(<div style={{background:"white",borderRadius:16,padding:16}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
        <div style={{fontWeight:700,fontSize:14}}>❓ Donde esta?</div>
      ...
      <div style={{background:"#FDE8F0",borderRadius:14,padding:14,textAlign:"center",marginBottom:12}}>
        ...
        <div style={{fontFamily:"Georgia,serif",fontSize:64,fontWeight:700,color:"#E8719C",lineHeight:1}}>{correctPh}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {opts.map((opt,i)=>(<button key={i} onClick={()=>{if(feedback)return;const ok=opt.ph===correctPh;setFeedback(ok?"ok":"mal");if(ok)setScore(s=>s+1);setTimeout(()=>{setFeedback(null);if(idx+1<ronda.length)setIdx(i=>i+1);else setJuegoActivo("fin");},900);}} style={{padding:"14px 8px",borderRadius:12,border:"2px solid #F9C8DC",background:"white",cursor:"pointer",textAlign:"center"}}>
          <div style={{fontSize:40}}>{opt.emoji}</div>
          <div style={{fontSize:11,fontWeight:700,color:"#2C2C2C",marginTop:4}}>{opt.word}</div>
        </button>))}
      </div>
      <button style={{marginTop:12,width:"100%",padding:10,borderRadius:10,border:"none",background:"#EDE0F5",color:"#6B6560",cursor:"pointer",fontWeight:600}} onClick={cerrar}>Salir</button>
    </div>);
  ...
  if(juegoActivo==="fin"){
    return(<div style={{background:"white",borderRadius:16,padding:24,textAlign:"center"}}>
      <div style={{fontSize:48,marginBottom:12}}>🎉</div>
      <div style={{fontFamily:"Georgia,serif",fontSize:24,fontWeight:700,marginBottom:8}}>Juego terminado!</div>
      <div style={{fontSize:16,color:"#9B9590",marginBottom:16}}>Puntaje: <strong style={{color:"#9B7EBD"}}>{score}</strong> de {ronda.length}</div>
      <div style={{background:"#F5F0FA",borderRadius:12,padding:12,marginBottom:16,fontSize:13}}>{score===ronda.length?"🌟 Perfecto!":score>=ronda.length/2?"👏 Muy bien!":"💪 Seguimos practicando!"}</div>
      <button style={{background:"#9B7EBD",color:"white",border:"none",borderRadius:12,padding:"12px 24px",fontSize:14,fontWeight:700,cursor:"pointer"}} onClick={cerrar}>Jugar de nuevo</button>
    </div>);
  }
  return(<div>
    {[{id:"atrapa",icon:"🎯",name:"Atrapa el sonido",desc:"Escucha el fonema y toca la letra correcta",color:"#9B7EBD",action:iniciarAtrapa},{id:"donde",icon:"❓",name:"Donde esta?",desc:"Ve el fonema y toca la imagen correcta",color:"#E8719C",action:iniciarDonde}].map(j=>(
      <div key={j.id} style={{display:"flex",alignItems:"center",gap:12,padding:12,background:"white",borderRadius:14,marginBottom:10,cursor:"pointer",border:`2px solid ${j.color}22`,boxShadow:"0 1px 6px rgba(0,0,0,.06)"}} onClick={j.action}>
        <div style={{width:48,height:48,borderRadius:13,background:j.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{j.icon}</div>
        <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14,color:"#2C2C2C"}}>{j.name}</div><div style={{fontSize:11,color:"#9B9590",marginTop:2}}>{j.desc}</div></div>
        <div style={{background:j.color,color:"white",padding:"6px 14px",borderRadius:8,fontSize:12,fontWeight:700}}>Jugar</div>
      </div>
    ))}
  </div>);
}
