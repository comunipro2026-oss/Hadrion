...
.phbtn{aspect-ratio:1;border-radius:12px;border:2px solid #EDE0F5;background:white;font-family:'Cormorant Garamond',serif;font-size:18px;font-weight:700;cursor:pointer;transition:all .17s;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#2C2C2C;gap:2px;min-height:54px;}
  ...
  return (
    <div className="fu">
      ...
      <div className="phgrid">
        {(cats[fil] || PHONEMES).map(ph => (
          <button key={ph} className={`phbtn${sel===ph?" sel":""}`} onClick={() => handle(ph)}>
            ...
            {PHONEME_EMOJI[ph] && <div style={{ fontSize:14, lineHeight:1 }}>{PHONEME_EMOJI[ph][0]}</div>}
          </button>
        ))}
      </div>
      ...
      <SC title="🎮 Juegos Fonologicos">
        {[
          ["🎯","Atrapa el sonido","Toca el fonema que escuchas entre 3 opciones."],
          ["❓","Donde esta?","El nino toca la imagen que empieza con el fonema indicado."],
          ["🃏","Parejas de sonido","Unir cada letra con su imagen. Basico: 4 pares. Avanzado: 8 pares."],
          ["✂️","Segmentacion","El nino separa silabas con palmadas o fichas de color."],
          ["🔀","Manipulacion","Cambiar el fonema inicial: PATO → GATO → MATO → DATO."],
        ].map(([ic, n, d]) => (
